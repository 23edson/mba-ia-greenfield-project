...
describe('VideoProcessingProcessor', () => {
  ...
  beforeEach(async () => {
    ...
    jest
      .spyOn(processor as any, 'convertToFaststart')
      .mockResolvedValue(undefined);
  });
  ...
  it('should process video successfully', async () => {
    const job = {
      data: {
        videoId: 'video-123',
        storageKey: 'uploads/video-123/original.mp4',
      },
    } as unknown as Job;

    videosRepository.findOneBy.mockResolvedValue({
      id: 'video-123',
      status: 'draft',
    });
    ...
    expect(videosRepository.update).toHaveBeenCalledWith('video-123', {
      status: 'processing',
    });
  ...
  it('should throw standard Error and NOT update status to error if infra/storage fails', async () => {
    ...
    storageService.downloadFile.mockRejectedValueOnce(
    ...
    expect(videosRepository.update).not.toHaveBeenCalledWith(
      'video-123',
      expect.objectContaining({
        status: 'error',
      }),
  ...
  it('should throw standard Error and NOT update status to error if uploadFile fails', async () => {
    const job = {
      data: {
        videoId: 'video-upload-fail',
        storageKey: 'uploads/video-upload-fail/original.mp4',
      },
    } as unknown as Job;

    videosRepository.findOneBy.mockResolvedValue({
      id: 'video-upload-fail',
      status: 'processing',
    });
    ...
    // Should only have been called to update to 'processing'
    expect(videosRepository.update).toHaveBeenCalledTimes(1);
    expect(videosRepository.update).toHaveBeenCalledWith('video-upload-fail', {
      status: 'processing',
    });
  });
});
