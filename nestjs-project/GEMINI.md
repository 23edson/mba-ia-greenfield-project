# GEMINI.md — nestjs-project

> Arquivo de instruções para o **Antigravity CLI (AGY / Gemini CLI)**.
> Portado a partir do `nestjs-project/CLAUDE.md` original do projeto (Claude Code).
...
During active development, run only the tests related to the file being changed (`npm test -- path/to/file.spec.ts`). Before declaring a task done, run the full suite — see the root `GEMINI.md` → "Definition of Done (Technical)".
...
Conventions for **how to write** each kind of test (mocking patterns, AAA structure, override strategies for global guards, etc.) live in `.agents/rules/nestjs-testing.md` and load when you edit a test file.
