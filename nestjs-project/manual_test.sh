...
echo "2. Pegando token de confirmação no Mailpit..."
sleep 2
MSG_ID=$(curl -s http://localhost:8025/api/v1/messages | jq -r '.messages[0].ID')
TOKEN=$(curl -s "http://localhost:8025/api/v1/message/$MSG_ID" | jq -r '.Text' | grep -o 'token=[A-Za-z0-9_-]*' | head -1 | cut -d= -f2)
...
echo "8. Aguardando 10 segundos para o worker processar..."
sleep 10
