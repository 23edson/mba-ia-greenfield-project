...
> Portado a partir do `CLAUDE.md` original do projeto (Claude Code).
...
Skills are located in `.agents/skills/`. The main workflow skills are:

| Skill | Description |
|---|---|
| `research` | Pesquisa opções e registra decisões técnicas |
| `plan-context` | Consolida o contexto da fase → `context.md` |
| `plan-validate` | Valida inconsistências → `validation.md` |
| `plan-resolve` | Resolve pendências e fixa libs → `library-refs.md` |
| `plan-build` | Gera o plano executável → `phase-XX.md` |
| `plan-test-specs` | Gera specs de teste |
| `implement` | Implementa passo a passo → código + `progress.md` |
